...
struct ARM7_RegisterSet
{
  ...
  inline bool GetFlag(ARM7_CpuFlag flag)
  {
    return BIT(CPSR, static_cast<int>(flag));
  }
  inline void SetFlag(ARM7_CpuFlag flag, bool on)
  {
    BIT_SET(CPSR, static_cast<int>(flag), on);
  }

  inline bool GetControlBit(ARM7_ControlBits bit)
  {
    return BIT(CPSR, static_cast<int>(bit));
  }
  inline void SetControlBit(ARM7_ControlBits bit, bool on)
  {
...
struct ARM7_Exception
{
  ...
  unsigned int Priority =
      0;  // 0 means nothing Priority for simultaneously occuring exceptions
          // ranges from Prio=1=Highest to Prio=7=Lowest.
  ARM7_OpMode ModeOnEntry = ARM7_OpMode::UND;
  std::string_view InterruptFlags =
      "--";  // I first and F second, values can be 1 0 or u for unchanged
};
...
static const std::array<ARM7_Exception, 8> sARM7_Exceptions = {
    ...
    ARM7_Exception{ARM7_ExceptionType::Undefined_Inst, 0x04, 7,
                   ARM7_OpMode::UND, "1u"},
    ARM7_Exception{ARM7_ExceptionType::Software_Int, 0x08, 6, ARM7_OpMode::SVC,
                   "1u"},
    ARM7_Exception{ARM7_ExceptionType::Prefetch_Abort, 0x0C, 5,
                   ARM7_OpMode::ABT, "1u"},
    ARM7_Exception{ARM7_ExceptionType::Data_Abort, 0x10, 2, ARM7_OpMode::ABT,
                   "1u"},
    ARM7_Exception{ARM7_ExceptionType::Address_Exceeds_26bit, 0x14, 0,
                   ARM7_OpMode::SVC, "1u"},
    ARM7_Exception{ARM7_ExceptionType::Normal_Int, 0x18, 4, ARM7_OpMode::IRQ,
                   "1u"},
    ARM7_Exception{ARM7_ExceptionType::Fast_Int, 0x1C, 3, ARM7_OpMode::FIQ,
                   "11"}};
...
#define ARM7_OpcodeCond(inst) ((inst >> 28) & 0x0000000F)

#define WIDTH(start, end) ((start) - (end) + 1)
#define MASK(start, end) ((1u << WIDTH(start, end)) - 1u)
#define BITS(value, start, end) (((value) >> (end)) & MASK(start, end))
#define TEST(value, start, end, equals) (BITS(value, start, end) == (equals))
...
enum class ARM7_InstructionOpcodeTypes
{
  B_BL,
  BX_BLX
};

struct ARM7_ARMInstruction
{
  u32 data;
};

enum class ARM7_ALUOpcode
{
  AND,
  EOR,
  SUB,
...
class ARM7TDMI
{
 ...
 private:
  void ExecuteARMState();
  void ExecuteThumbState();

  void ExARM_SWP(const ARM7_ARMInstruction& instruction);   // Data Swap Word
  void ExARM_SWPB(const ARM7_ARMInstruction& instruction);  // Data Swap Byte
  void ExARM_UND(const ARM7_ARMInstruction& instruction);   // Undefined
  void ExARM_SWI(const ARM7_ARMInstruction& instruction);  // Software Interrupt

  void ExARM_B(const ARM7_ARMInstruction& instruction);
  void ExARM_BL(const ARM7_ARMInstruction& instruction);

  void ExARM_BX(const ARM7_ARMInstruction& instruction);

  void ExARM_ALU(const ARM7_ARMInstruction& instruction);

  void ExARM_Multiply(const ARM7_ARMInstruction& instruction);
  void ExARM_MUL(const ARM7_ARMInstruction& instruction);
  void ExARM_MLA(const ARM7_ARMInstruction& instruction);
  void ExARM_MULL(const ARM7_ARMInstruction& instruction);
  void ExARM_MLAL(const ARM7_ARMInstruction& instruction);

  void ExARM_PSRTransfer(const ARM7_ARMInstruction& instruction);
